...
extension UIBezierPath{
    class func makePoint(originalFrame : CGRect, length : CGFloat)-> UIBezierPath{
        ...
        bezierPath.addLineToPoint(CGPointMake(rect.width, rect.height-2*length))
        bezierPath.addArcWithCenter(CGPointMake(rect.width-length,rect.height-2*length), radius: length, startAngle: 0, endAngle: CGFloat(M_PI_2), clockwise: true)
        ...
        bezierPath.addLineToPoint(CGPointMake(rect.width/2-length, rect.height-length))
        bezierPath.addLineToPoint(CGPointMake(length, rect.height-length))
        bezierPath.addArcWithCenter(CGPointMake(length,rect.height-2*length), radius: length, startAngle: CGFloat(M_PI_2), endAngle: CGFloat(M_PI), clockwise: true)
...
extension CAShapeLayer{
    func getFromPath(path:UIBezierPath,color:UIColor=UIColor.blackColor())->CAShapeLayer{
...
extension CALayer{
    func cleanLayer(){
        ...
        path.moveToPoint(CGPointMake(self.frame.origin.x,self.frame.height/2))
        path.addLineToPoint(CGPointMake(self.frame.width,self.frame.height/2))
        path.closePath()
        let cleanLayer = CAShapeLayer()
        cleanLayer.path = path.CGPath
        cleanLayer.strokeColor = mainBgColor.CGColor
        cleanLayer.lineWidth = self.frame.height
        self.addSublayer(cleanLayer)
    }
}