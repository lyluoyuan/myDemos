...
func caculateJointForTwoPoint(point1:CGPoint,point2:CGPoint)->(CGPoint,CGPoint){
    let yRange = point2.y-point1.y
    let xRange = point2.x-point1.x
    let length = sqrt(xRange*xRange + yRange*yRange)
    let startPoint = CGPointMake(point1.x+4.5/length*xRange, point1.y+4.5/length*yRange)
    let endPoint = CGPointMake(point2.x-4.5/length*xRange, point2.y-4.5/length*yRange)
